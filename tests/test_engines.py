...
def test_attack_excludes_low_fit_prospect(fixture_repo):
    candidates = attack.generate_candidates(fixture_repo, "R100", DEMO_DATE)
...
def test_defend_requires_strong_trigger_or_two_general_signals(fixture_repo):
    candidates = defend.generate_candidates(fixture_repo, "R100", DEMO_DATE)
    ids = [c.target_id for c in candidates]
...
def test_no_banned_phrases_in_any_engine_output(fixture_repo):
    all_candidates = (
        attack.generate_candidates(fixture_repo, "R100", DEMO_DATE)
        + defend.generate_candidates(fixture_repo, "R100", DEMO_DATE)
        + grow.generate_candidates(fixture_repo, "R100", DEMO_DATE)
    )
...
def test_grow_requires_at_least_one_signal(fixture_repo):
    candidates = grow.generate_candidates(fixture_repo, "R100", DEMO_DATE)
    ids = [c.target_id for c in candidates]
    for filler_id in ("A007", "A008", "A009", "A010"):
        assert filler_id not in ids
